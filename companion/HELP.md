...
- Set Position and Size
	...
	Unlike WebRCS this action does not always refer the layer position from the center of the layer but offers detailed settings for the anchor point in X and Y direction inside or outside of the layer.
	...
	For the anchor point you will usually want the center of the layer, this is the same behavior as WebRCS and given as default entries. The anchor points have to be entered in pixels as well and so the horizontal center is to be calculated by the layer x position + half of the layer width. The expression is 'lx + 0.5 * lw'. For the vertical anchor it is 'ly + 0.5 * lh'.
	If you do only resizing, the anchor position stays at the same spot and everything else will be resized. If your anchor position is outside of the layer area, the layer will also appear to move. Think of it as also scaling the distance from the anchor point to the layer.  
	...
	If you choose to touch x or y position, the anchor point will be positioned at the according value.
	...
	IF you want to scale many layers and want them to keep there relative positions, you have to use the keywords for the bounding box of the layers, e.g. you want them to be scaled up by 5% from the middle of the layers use 'bx + 0.5 * bw' for the Anchor X. For the width you can use 'lw * 1.05'
