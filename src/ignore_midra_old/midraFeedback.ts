...
import { combineRgb, CompanionFeedbackBooleanEvent, CompanionFeedbackDefinitions, CompanionOptionValues } from '@companion-module/base'
...
export function getFeedbacks(instance: AWJinstance): CompanionFeedbackDefinitions {
	...
	feedbacks['presetToggle'] = {
		...
		options: [],
		callback: (_feedback) => {
			return state.get('DEVICE/device/screenGroupList/items/S1/control/pp/enablePresetToggle') === true
		},
	}
	...
	feedbacks['deviceScreenMemory'] = {
		...
		callback: (feedback: CompanionFeedbackBooleanEvent & { options: { screens: string[], preset: string, memory: string, unmodified: number } }) => {
			...
			const screens = choices.getChosenScreens(feedback.options.screens)
			...
			for (const screen of screens) {
				for (const preset of presets) {
					if (
						...
						if ( ((!false && modified) || (false && !modified)) == feedback.options.unmodified) {
							return true
						}
					}
				}
...
// MARK: Aux Memory
	feedbacks['deviceAuxMemory'] = {
		...
		callback: (feedback: CompanionFeedbackBooleanEvent & { options: { screens: string[], preset: string, memory: string, unmodified: number } }) => {
			...
			const screens = choices.getChosenAuxes(feedback.options.screens)
			const presets = feedback.options.preset === 'all' ? ['pgm', 'pvw'] : [feedback.options.preset]
			for (const screen of screens) {
				for (const preset of presets) {
					if (
		...
		options: [
			{
				...
				default: getScreenAuxChoices(state)?.[0] || 'S1'
			},
		],
		...
		options: [
			...
			...[{id:'any'}, ...getScreensArray(state)].map((screen) => {
				const opt = {
					...
					default: '1',
					isVisible: (options: CompanionOptionValues, screenId = screen.id) => {return options.screen === screenId},
				}
				if (screen.id === 'any') {
