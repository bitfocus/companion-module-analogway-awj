...
export function getFeedbacks(instance: AWJinstance): CompanionFeedbackDefinitions {
	...
	feedbacks['presetToggle'] = {
		...
		options: [],
		callback: (_feedback) => {
			return state.get('DEVICE/device/screenGroupList/items/S1/control/pp/copyMode') === false
		},
	}
	...
	feedbacks['deviceGpioOut'] = {
		...
		callback: (feedback: CompanionFeedbackBooleanEvent & { options: { gpo: number, state: number } }) => {
			...
			return (
				state.get([
					'DEVICE', 'device', 'gpio',
					'gpoList', 'items', feedback.options.gpo?.toString() || '1',
					'status', 'pp', 'state',
				]) === val
			)
		},
	...
	feedbacks['deviceGpioIn'] = {
		...
		callback: (feedback: CompanionFeedbackBooleanEvent & { options: { gpi: number, state: number } }) => {
			...
			return (
				state.get([
					'DEVICE', 'device', 'gpio',
					'gpiList', 'items', feedback.options.gpo?.toString() || '1',
					'status', 'pp', 'state',
				]) === val
			)
		},
